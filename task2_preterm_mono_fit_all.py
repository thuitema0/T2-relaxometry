...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Task 2: mono-exponential T2 fitting (weighted LS, plain LS, NNLS grid, NLLS)"
    )
    parser.add_argument(
        "--data-root", type=str, required=True,
        help="Path to the folder containing NIfTI images, masks, segs, and TEs.txt",
    )
    parser.add_argument(
        "--output-root", type=str, required=True,
        help="Path to the output folder for fitted maps and summaries",
    )
    parser.add_argument(
        "--subjects", type=str, nargs="*", default=None,
        help="Subject IDs to process (default: auto-discover all subjects in data-root)",
    )
    parser.add_argument(
        "--methods", type=str, nargs="*",
        default=["ls", "weighted_ls", "nnls_grid", "nlls"],
        help="Fitting methods to run (default: ls weighted_ls nnls_grid nlls)",
    )
...
OUTPUT_ROOT: Path = None  # type: ignore
...
# ============================================================
# Method 1b: plain LS (log-domain OLS, no weighting)
# ============================================================

def fit_plain_ls(signals: np.ndarray, tes: np.ndarray):
    N, E = signals.shape
    x = tes.astype(np.float32)
    ...
    x_mean = np.mean(x)
    y_mean = np.mean(logy, axis=1)

    den = np.sum((x - x_mean) ** 2)
    num = np.sum((x[None, :] - x_mean) * (logy - y_mean[:, None]), axis=1)
    ...
    pred = s0_fit[:, None] * np.exp(-x[None, :] / np.clip(t2_fit[:, None], EPS, None))
    rss = np.sum((pred - y) ** 2, axis=1)
    denom = np.sum(y ** 2, axis=1)
    nrmse_fit = np.sqrt(rss / np.clip(denom, EPS, None))
...
def process_subject(subject_id: str, tes: np.ndarray, logfile: Path, methods_to_run: list):
    ...
    for method in methods_to_run:
        ...
        if method == "ls":
            s0_vec, t2_vec, nrmse_vec, valid_vec = fit_plain_ls(signals, tes)
...
def main():
    ...
    for sid in subject_ids:
        try:
            all_rows.extend(process_subject(sid, tes, logfile, methods_to_run))
    ...
    if len(all_rows) > 0:
        ...
        summary_path = OUTPUT_ROOT / "all_methods_all_subjects_summary.csv"
        # Append to existing summary (keep results from previous runs)
        if summary_path.exists():
            old_df = pd.read_csv(summary_path)
            combined = pd.concat([old_df, all_df], ignore_index=True)
            combined = combined.drop_duplicates(
                subset=["subject_id", "method"], keep="last"
            )
            combined.to_csv(summary_path, index=False)
        else:
            all_df.to_csv(summary_path, index=False)
        log(f"Saved: {summary_path}", logfile)
