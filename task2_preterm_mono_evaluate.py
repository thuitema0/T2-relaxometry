...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Task 2: evaluate mono-exponential T2 fitting results per tissue"
    )
    parser.add_argument(
        "--data-root", type=str, required=True,
        help="Path to the folder containing NIfTI images, masks, and segs",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Methods to evaluate (default: ls weighted_ls nnls_grid nlls)",
    )
...
def main():
    global DATA_ROOT, FIT_ROOT, OUT_ROOT, METHODS
    args = parse_args()
